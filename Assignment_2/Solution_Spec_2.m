...
cell_data = importdata('BA_Spec_2.txt');                      % read  file to string (this func. can store every lines
...
condition_1 = extract(str_new, "(!o1) -> goto T0_init");
condition_2 = extract(str_new, "(r1 && !o1) -> goto accept_S2");
...
if contains(str_new, condition_1)                   % if satisfy condition_1 (never go to o1)
   ...
   % ---- condition_2 ---- %
   if contains(str_new, condition_2)                % only if satisfy condition_1, then have condition_2
       ...
       for n=1:9                                    % eventually would get to r1
           if P(n,1)~= 0                    
               P(n,1) = 1;
           end
           if P(1,n)~= 0                            % not going other states
              P(1,n) = 0;
           end
       end
   end
...
numSteps = 15;                  
disp("Randomly walk, and satisfy the specification 2: ")
